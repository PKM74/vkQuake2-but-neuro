...
static void default_logger(neurosdk_severity_e severity, char *message,
                           void *user_data) {
#ifdef _WIN32
...
NEUROSDK_EXPORT char const *neurosdk_version(void) { return STR(LIB_VERSION); }
...
static neurosdk_error_e parse_s2c_json(context_t *ctx, neurosdk_message_t *msg,
                                       char const *json, int len) {
  if (!ctx) {
...
NEUROSDK_EXPORT neurosdk_error_e neurosdk_context_create(
    neurosdk_context_t *ctx, neurosdk_context_create_desc_t *desc) {
  neurosdk_error_e res = NeuroSDK_None;
...
neurosdk_context_poll(neurosdk_context_t *ctx,
                      OUT neurosdk_message_t **messages, OUT int *count) {
  if (!ctx || !(*ctx)) {
    return NeuroSDK_Uninitialized;
  }
  context_t *context = (context_t *)(*ctx);
...
neurosdk_context_send(neurosdk_context_t *ctx, neurosdk_message_t *msg) {
  ...
  case NeuroSDK_MessageKind_Context: {
    ...
    if (!escaped_str) {
      LOG_ERROR(context, "Out of memory while escaping 'message' for context.");
      return NeuroSDK_OutOfMemory;
    }
    bytes = aprintf(&str,
  ...
  case NeuroSDK_MessageKind_ActionsRegister: {
    if (msg->value.actions_register.actions_len <= 0) {
      LOG_WARN(context, "MessageKind_ActionsRegister called with zero actions. "
                        "Nothing to register?");
    }
  ...
  case NeuroSDK_MessageKind_ActionsUnregister: {
    if (msg->value.actions_unregister.action_names_len <= 0) {
      LOG_WARN(context, "MessageKind_ActionsUnregister called with zero action "
